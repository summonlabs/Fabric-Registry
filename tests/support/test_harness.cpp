// Fabric Registry — minimal dependency-free test harness implementation.
//
...
class CaseFailure final : public std::exception {
 public:
  const char* what() const noexcept override { return "frtest case failure"; }
};
...
int run_all(int argc, char** argv) {
  ...
  for (int index = 1; index < argc && argv != nullptr; ++index) {
    ...
    const bool next_is_value =
        index + 1 < argc && argv[index + 1] != nullptr && std::string_view(argv[index + 1]).rfind("--", 0) != 0;
...
std::int64_t Rng::in_range(std::int64_t lo, std::int64_t hi) noexcept {
  ...
  const std::uint64_t offset = span == ~static_cast<std::uint64_t>(0) ? next_u64() : below(span + 1u);
