// Fabric Registry — minimal dependency-free test harness.
//
...
#ifndef FABRIC_REGISTRY_TESTS_SUPPORT_TEST_HARNESS_HPP
#define FABRIC_REGISTRY_TESTS_SUPPORT_TEST_HARNESS_HPP
...
#define FR_TEST_CASE(suite, name)                                               \
  static void frtest_case_##suite##_##name();                                   \
  namespace {                                                                   \
  const bool frtest_registered_##suite##_##name =                               \
      (::frtest::register_case(#suite, #name, &frtest_case_##suite##_##name),   \
       true);                                                                   \
  } /* namespace */                                                             \
  static void frtest_case_##suite##_##name()

/// Aborts the running case unless the expression is true.
#define FR_CHECK(expr)                                            \
  do {                                                            \
    if (!(expr)) {                                                \
      ::frtest::fail(__FILE__, __LINE__, "check failed: " #expr); \
      return;                                                     \
    }                                                             \
  } while (0)

/// Aborts the running case unless the two operands are equal.
#define FR_CHECK_EQ(a, b)                                                \
  do {                                                                   \
    if (!((a) == (b))) {                                                 \
      ::frtest::fail(__FILE__, __LINE__, "check failed: " #a " == " #b); \
      return;                                                            \
    }                                                                    \
  } while (0)

/// Aborts the running case unless the expression is true, reporting a message.
#define FR_CHECK_MSG(expr, msg)                  \
  do {                                           \
    if (!(expr)) {                               \
      ::frtest::fail(__FILE__, __LINE__, (msg)); \
      return;                                    \
    }                                            \
  } while (0)
...
#endif // FABRIC_REGISTRY_TESTS_SUPPORT_TEST_HARNESS_HPP
