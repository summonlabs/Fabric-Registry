...
ChildProcess::~ChildProcess() {
  ...
  if (impl_->stdout_write != nullptr) {
    CloseHandle(impl_->stdout_write);
  }
  if (impl_->stdin_read != nullptr) {
    CloseHandle(impl_->stdin_read);
  }
...
std::optional<ChildProcess> ChildProcess::spawn(const std::string& program,
  ...
  if (created == 0) {
    ...
    error = "the child process could not be created (windows error " + std::to_string(GetLastError()) + ")";
    return std::nullopt;
  }
  ...
  child.impl_->reader = std::thread([impl = child.impl_.get()]() {
    char buffer[4096];
    DWORD produced = 0;
    while (ReadFile(impl->stdout_read, buffer, sizeof(buffer), &produced, nullptr) != 0 && produced != 0) {
...
std::string make_temporary_directory(const std::string& label) {
  ...
  for (int attempt = 0; attempt < 1000; ++attempt) {
    const std::filesystem::path candidate =
        base / ("fabric-registry-test-" + label + "-" + std::to_string(attempt));
    std::error_code error;
